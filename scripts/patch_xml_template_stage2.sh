...
# Usage: patch_xml_template_stage2.sh host port username password mart dataset filterName featureName filterTitle featureTitle linkoutURL featureDefault
...
# this can happen if no tables were added to ensembl, in which case there are no $dbprefix hits..
if [[ $tblname == '' ]] ; then
...
column2=`echo -e "SHOW COLUMNS FROM $mart.$tblname;\n" | mysql -h $host -P $port -u $user --password=$pass | grep -v -E "(([a-zA-Z0-9_]+_key)|(taxonomy_id_[[:digit:]]+)|(dbprimary_acc_[[:digit:]]+)|(stable_id_[[:digit:]]+))" | tail -n 1 | awk -F' ' '{ print $1 }'`

if [ $? -ne 0 ] ; then
	exit $ERR_SQL_SHOW
fi

sed 's@FILTER_LIMIT_PLACEHOLDER@+          <Option displayName="with '"$filterTitle"'" displayType="list" field="'"$dbprefix"'_'"$filterName"'_bool" internalName="with_'"$dbprefix"'_'"$filterName"'" isSelectable="true" key="'"$column1"'" legal_qualifiers="only,excluded" qualifier="only" style="radio" tableConstraint="main" type="boolean">\
